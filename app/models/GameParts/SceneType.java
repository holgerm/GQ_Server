...
canSeeHotspots = true;
    ...
    Integer sortstart = 10000;
    	for(Attribute aa: g.getAllSubAttributes()){
    		...
    		log = log + "Attribut mit ID"+aa.getType().getId()+", XMLTyp "+aa.getXMLType()+" und Value "+aa.getValue();
    		if(aa.getValue() != null){
    		if(aa.getValue().contains("scenefield(")){
    			sortstart = sortstart +1;
    			log = log + " -> Link: "+aa.getValue();
    			String split = aa.getValue();
    			...
    			if(split3.length > 2){
        			...
        			sort = Integer.valueOf(split3[2]);
        				...
        				log = log +"Couldn't transform sort-Attribute to Integer value: "+ split3[2];
        	...
        	String defaultvalues = 	split3[3];
    			...
    			try{
    					AttributeType att01 = new AttributeType(field,String.valueOf(aa.getId()),type);
...
log = log+"<h1>Sorter</h1>";	
    		...
    		for(Integer key : sorter.keySet()) {
    			...
    			log = log + "<br/>" + key + ":  "+sorter.get(key).getName()+" ("+sorter.get(key).getId()+")";
...
log = log+"</ul><br/><br/>";
    ...
    public void setSeeRules(boolean x){ canSeeRules = x; }
    public void setSeeMissions(boolean x){ canSeeMissions = x; }
    public void setSeeHotspots(boolean x){ canSeeHotspots = x; }

    public void setAddHotspots(boolean x){ canAddHotspots = x; }
    public void setAddRules(boolean x){ canAddRules = x; }
    public void setAddMissions(boolean x){ canAddMissions = x; }

    
    public void setIcon(String x){ icon = x; }
    public void setIconOpen(String x){ iconOpen = x; }
    
    public void setSeeChildren(boolean x){ seeChildren = x; }
...
public void addDefaultPart(Part p){ defaultParts.add(p); }

public void addPossiblePartTypes(PartType x){ possiblePartTypes.add(x); System.out.println("Trying to add possible Part Type."); }
public void addPossibleHotspotTypes(HotspotType x){ possibleHotspotTypes.add(x); }
public void addPartOccurence(PartOccurrence x){ partPermissions.add(x); }
public void addDefaultHotspots(Hotspot x){ defaultHotspots.add(x); }
...
public boolean canSeeMissions(){ return canSeeMissions; }
public boolean canSeeRules(){ return canSeeRules; }
public boolean canSeeHotspots(){ return canSeeHotspots; }



public String getIcon(){ return icon; }
public String getIconOpen(){ return iconOpen; }
public boolean canSeeChildren(){ return seeChildren; }

public List<AttributeType> getAttributeTypes(){ return attributeTypes; }
public List<Part> getDefaultPart(){ return defaultParts; }

public List<PartType> getPossiblePartTypes(){ return possiblePartTypes; }
public List<HotspotType> getPossibleHotspotTypes(){ return possibleHotspotTypes; }
public List<PartOccurrence> getPartPermissions(){ return partPermissions; }
public List<Hotspot> getDefaultHotspots(){ return defaultHotspots; }
...
System.out.println("Ersetze alle $_"+entry.getKey().getId()+" durch $_"+entry.getValue().getId());
    	...
    	for(Attribute a:s.getAllSubAttributes()){
    		...
    		if(a.getValue().contains("$_")){
    			...
    			newvalue = newvalue.replace("$_mission_"+entry.getKey().getId(), "$_mission_"+entry.getValue().getId());
    			System.out.println("excecuting in "+a.getType().getName()+" of type "+a.getTypeDescription()+":$_"+ entry.getKey().getId()+" -> $_"+entry.getValue().getId());
    		...
    		if(a.getType().getFileType().equals("mission")){
    			...
    			System.out.println("excecuting mission attribute:"+ entry.getKey().getId()+" -> "+entry.getValue().getId());

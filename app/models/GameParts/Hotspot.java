...
public class Hotspot extends Model {
    ...
    public void setParent(Hotspot x){ parent = x; }
...
Attribute x = null;
    ...
    public static final Finder<Long, Hotspot> find = new Finder<Long, Hotspot>(
            Long.class, Hotspot.class);
		...
		for(AttributeType aa:getAllAttributes()){
			...
			if(aa.getFileType().equals("file")){
				...
				if(!file.exists()){
					...
					file = new File("public/uploads/"+ Application.getLocalPortal().getId() + "/editor/"+g.getId()+"/files",path);
